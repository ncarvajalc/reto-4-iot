...
class RemaView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        measureParam = self.kwargs.get('measure', None) # Obtiene el parámetro de la url
        ...
        try:
            start = datetime.fromtimestamp(
                float(self.request.GET.get('from', None))/1000) # Obtiene el query param 'from' y lo convierte a datetime
        except:
            start = None
        try:
            end = datetime.fromtimestamp(
                float(self.request.GET.get('to', None))/1000) # Obtiene el query param 'to' y lo convierte a datetime
        except:
            end = None
...
def get_map_json_2(request, **kwargs):
    ...
    locations = Location.objects.all()
    # Use 10/07/2021
    end = datetime(2021, 7, 10, 23, 59, 59) # Idealmente se debería usar la fecha actual (con datetime.now()),
                                            # pero por ahora se fijará a una fecha estática por la carga de datos.
    start = end - timedelta(days=1) # Obtiene el último día

    data = []

    for location in locations:
        stations = Station.objects.filter(location=location, last_activity__gte=start)
    ...
    startFormatted = start.strftime("%d/%m/%Y %H:%M:%S")
    endFormatted = end.strftime("%d/%m/%Y %H:%M:%S")
